...
class Playlist:
    def __init__(
        ...
        if track is False:
            ...
            recomendations = spotify_client.recommendations(
                seed_genres=genres,
                limit=100,
        ...
        self.total_duration = milliseconds_to_string_duration(
            sum([song["duration_ms"] for song in self.songs])
        )
...
def songs(playlist_id):
    ...
    if not song_to_delete:
        ...
        playlists_ = [literal_eval(playlist) for playlist in playlists_]
        playlist = [
            playlist for playlist in playlists_ if playlist["id"] == int(playlist_id)
        ][0]
        ...
        if len(playlist["songs"]) > 1:
            playlist["songs"] = [
                ...
                if song["url"] != song_to_delete["url"]
            ]
            playlist["total_duration"] = milliseconds_to_string_duration(
                sum(song["duration_ms"] for song in playlist["songs"])
            )
...
def generate():
    if request.method == "POST":
        if request.form["action"] == "generate":
            ...
            instrumentalness = (
                int(request.form["instrumentalness"]) * 0.01
                if request.form["instrumentalness"] != "0.5"
                else None
            )
            energy = (
                int(request.form["energy"]) * 0.01
                if request.form["energy"] != "0.5"
                else None
            )
            danceability = (
                int(request.form["danceability"]) * 0.01
                if request.form["danceability"] != "0.5"
                else None
            )
            valence = (
                int(request.form["valence"]) * 0.01
                if request.form["valence"] != "0.5"
                else None
            )
            popularity = (
                int(request.form["popularity"])
                if request.form["popularity"] != "50"
                else None
            )
            acousticness = (
                int(request.form["acousticness"]) * 0.01
                if request.form["acousticness"] != "0.5"
                else None
            )
            try:
                playlist = Playlist(
            ...
            return render_template(
                "generate.html", playlist=playlist, section="playlist"
            )
            ...
            playlist["songs"] = [
                song for song in playlist["songs"] if song["url"] != deleted_song["url"]
            ]
...
def add_to_spotify():
    oauth_manager = SpotifyOAuth(
        ...
        redirect_uri=url_for("add_to_spotify", _external=True),
    ...
    if request.method == "POST":
        ...
        created_playlist = spotify_user.user_playlist_create(
            user["id"],
            playlist["name"],
            public=False,
            description=f'Generated by TempoTune on {playlist["date"]}',
        )
